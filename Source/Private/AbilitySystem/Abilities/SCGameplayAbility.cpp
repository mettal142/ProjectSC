...
ASCCharacterBase* USCGameplayAbility::GetCharacterFromActorInfo()
{
	if (!CachedSCCharacter.IsValid())
	{
		CachedSCCharacter = Cast<ASCCharacterBase>(CurrentActorInfo->AvatarActor);
	}
	return  CachedSCCharacter.IsValid() ? CachedSCCharacter.Get() : nullptr;
}

...
bool USCAbilitySystemLibrary::MakeWidgetControllerParams(const UObject* WorldContextObject, FWidgetControllerParams& OutWCParams, ASCHUD*& OutSCHUD)
{
	if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContextObject, 0))
	{
		...
		if (OutSCHUD)
		{
			...
			OutSCHUD->GetWidgetControllerParams(OutWCParams);
			return true;
		}
	}
...
UQuickSlotWidgetController* USCAbilitySystemLibrary::GetQuickSlotWidgetController(const UObject* WorldContextObject)
{
	...
	if (MakeWidgetControllerParams(WorldContextObject, WCParams, SCHUD))
	{
		return SCHUD->GetQuickSlotWidgetController();
	}
	return nullptr;
}

void USCAbilitySystemLibrary::GetLiveCharactersWithinRadius(const UObject* WorldContextObject, TArray<AActor*>& OutOverlappingActors, const TArray<AActor*>& ActorsToIgnore, float Radius, const FVector& SphereOrigin)
{
